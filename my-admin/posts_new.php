...
if(isset($_POST['posts_new_create'])) {
	if(!empty($_POST['posts_title'])){
			if(!empty($_POST['posts_content'])){
				...
				$my_db->query("INSERT INTO my_blog (postTITLE,postCONT,postDATE,postAUTHOR,postCATEGORY,postPOSTED,postPERMALINK) VALUES ('$posts_title', '$posts_content', '$date', '$author', '$category', '1', '$permalink')");

				$info = '<div class="row"><div class="alert alert-success">'.ea('page_posts_new_success_posted', '1').' <a href="'.$permalink.'">'.ea('page_posts_new_success_show', '1').'</a></div>';

			} else {
				define("INDEX_ERROR", ea('page_posts_new_error_content', '1'));
			...
			$posts['content'] = $_POST['posts_content'];
			$posts_category = $_POST['category'];
			define("INDEX_ERROR", ea('page_posts_new_error_title', '1'));
...
.panel-heading .accordion-toggle.collapsed:after {
	...
	<div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                	<?php echo '<br>'.$info.'<br>'; ?>
                    <h1 class="page-header"><?php ea('page_posts_new_header'); ?></h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
            ...
            <div class="row">
               	<div class="col-lg-8">
               		<div class="form-group">
                        <label><?php ea('page_posts_new_title'); ?></label>
                       		...
                       		<div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h4 class="panel-title">
                                            <a data-toggle="collapse" data-parent="#accordion" href="#collapseOne" class="accordion-toggle"><?php ea('page_posts_new_publish'); ?></a>
                                        </h4>
                                    </div>
                                    <div id="collapseOne" class="panel-collapse collapse in" style="">
                                        ...
                                        <p id="msg" style="word-wrap: break-word; ">{@siteURL@}/blog/<?php echo date('Y', time());?>/<?php echo date('m', time());?>/</p> <br />
                                        <small>*<?php ea('page_posts_new_permalink_info'); ?></small>
                                        </div>
                                    </div>
                                     <div class="panel-footer"><button type="submit" name="posts_new_create" class="btn btn-info"><?php ea('page_posts_new_publish_button'); ?></button></div>
                        	</div>
               			</div>
                        <div class="form-group">
                       		<div class="panel panel-default">
                                    <div class="panel-heading">
                                        <h4 class="panel-title">
                                            <a data-toggle="collapse" data-parent="#accordion" href="#collapseTwo" class="accordion-toggle"><?php ea('page_posts_new_category'); ?></a>
                                        </h4>
                                    </div>
                                    <div id="collapseTwo" class="panel-collapse collapse in" style="">
                                        <div class="panel-body">
                                        <span class="label label-warning"><?php ea('page_posts_new_select_category'); ?></span><br /><br />
