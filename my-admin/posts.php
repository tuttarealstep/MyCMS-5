...
?>
	<div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                <?php echo '<br>'.$info.'<br>'; ?>
                    <h1 class="page-header"><?php ea('page_posts_header'); ?> <a href="{@siteURL@}/my-admin/posts_new" class="btn btn-primary"><?php ea('page_posts_header_create_new'); ?></a></h1> 
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
            <div class="row">
                 ...
                 <form action="" method="post">
                                <table class="table table-striped table-bordered table-hover" id="tables_posts">
                                    ...
                                    <tbody>
                                        <?php
											global $my_db;
											$post = $my_db->query("SELECT * FROM my_blog WHERE postPOSTED = '1'");
											$i = 0; foreach($post as $postinfo){ $i++; 
										?>
											<tr>
												<td><a href="{@siteURL@}/my-admin/posts_edit/<?php echo $postinfo['postID']; ?>"><?php echo $postinfo['postTITLE']; ?></a></td>
												<td><?php echo remove_space($postinfo['postAUTHOR']); ?></td>
												<td><?php echo $postinfo['postCATEGORY']; ?></td>
												<td><?php echo $postinfo['postDATE']; ?></td>
												<td><input type="checkbox" name="check_list[]" value="<?php echo $postinfo['postID']; ?>"></td>	
											</tr>
										<?php
											}
										?>
               ...
               <div class="col-lg-3">
               <p>&nbsp;</p>
               <button type="submit" name="execute" class="btn btn-danger"><?php ea('page_posts_check_button');?></button></div>
               </form>
         	</div>
            </div>
            <!-- /.row -->
