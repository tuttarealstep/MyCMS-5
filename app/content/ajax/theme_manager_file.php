...
if(staff_logged_in()) {
    ...
    <?php if($request_folder != "") { ?><li class="list-group-item"><i class="fa fa-folder"></i> <a href="#" onclick="go_to_folder_home();">.</a></li><?php } ?>
    <?php if($sub_sub_folder == true) { ?><li class="list-group-item"><i class="fa fa-folder"></i> <a href="#" onclick="show_folder('<?php echo s_crypt($path_back);?>');">..</a></li><?php } ?>
    <?php
    $dir_array = [];
    $file_array = [];
    ...
    foreach($dir_array as $dir_row){
        ?>
        <li class="list-group-item"><i class="fa fa-folder"></i> <a href="#<?php if($request_folder == "") { echo $dir_row; } else { echo '/'.$request_folder.'/'.$dir_row; }?>" onclick="show_folder('<?php if($request_folder == "") { echo s_crypt($dir_row); } else { echo s_crypt('/'.$request_folder.'/'.$dir_row); } ?>')" style="color: #563d7c;" ><b><?php echo $dir_row;?></b></a></li>
    <?php }
    foreach($file_array as $file_row){ ?>
        <li class="list-group-item"><i class="fa fa-file"></i> <a href="#<?php echo $file_row;?>" onclick="show_file('<?php echo s_crypt($file_row);?>', '<?php echo s_crypt(str_replace('//', '/', $request_folder)); ?>')"><?php echo $file_row;?></a></li>
    <?php }
?>
<script>
